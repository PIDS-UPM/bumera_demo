...
class Camera:
    def __init__(self, id:int, frame_width:int, frame_height:int, use:str):    #model
    ...
    def save_video(self, filename=datetime.strftime(datetime.now(), '%y_%m_%d__%H_%M_%S'), fs=20.0):
        if self.filming:
            fourcc = cv.VideoWriter_fourcc(*'mp4v')
            out = cv.VideoWriter('videos/'+filename+'.mp4', fourcc, fs, (self.frame_width, self.frame_height))
