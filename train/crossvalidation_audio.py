...
# Project: Bumera
# Date: 17/12/2024
# Description: This script processes audio files from the SAVEE dataset to extract 
#              features for emotion recognition. It applies various audio 
#              transformations (e.g., pitch shifting, time stretching, noise addition) 
#              for data augmentation. The features extracted include MFCCs, chroma 
#              features, pitch, spectral centroid, zero-crossing rate, and root mean 
#              square energy. These features are then used to train and evaluate 
#              different machine learning models (Random Forest, Decision Tree, 
#              Gradient Boosting, K-Nearest Neighbors, and Neural Networks) for 
#              emotion classification. The best hyperparameters for each model are 
#              determined through GridSearchCV, and performance metrics, including 
    ...
    for key, emotion in emotion_map.items():
        if key in audio_file:
            ...
            for aug_type in ["pitch_shift", "time_stretch", "add_noise"]:
                ...
                data.append(
                    [f"{audio_file.split('.')[0]}_{aug_type}", emotion]
                    + list(augmented_descriptors)
                )
            break


# Create the SAVEE dataset
columns = ["Audio", "Emotion"] + [
    f"Descriptor_{i}" for i in range(1, 1 + len(descriptors))
]
...
X = SAVEE_df_augmented.drop(
    columns=["Audio", "Emotion"], inplace=False
)  # Characteristics
...
param_grids = {
    ...
    "Neural Network (MLP)": {
        "hidden_layer_sizes": [(30,), (10, 10), (15, 15), (30, 30)],
...
with open(
    "train/audio_results.txt", "w"
) as f:  # File to store the best results and hyperparams
    for model_name, model in models.items():
        ...
        f.write(
            f"{model_name} - Prediction Time: {end_predict - start_predict:.2f} seconds\n"
        )

        if model_name in ["Gradient Boosting", "Neural Network (MLP)"]:
